...
namespace paydemo.con
{
    ...
    public sealed class RSAFromPkcs8
    {
        ...
        {
            byte[] dataKey = Convert.FromBase64String(privateKey);
        ...
        public static string sign(string content, string privateKey, string input_charset)
        {
            ...
            {
                MyLogger.WriteMessage(ex.ToString());
                return "";
            }
        ...
        public static bool verify(string content, string signedString, string publicKey, string input_charset)
        {
            ...
            using (RSA rsa = RSA.Create())
            {
                ...
                result = rsa.VerifyData(Data, data, HashAlgorithmName.SHA1, RSASignaturePadding.Pkcs1);
            }
            return result;
        }
        ...
        /// <param name="input_charset">编码格式</param>  
        /// <returns>明文</returns>  
        public static string Encrypt(string resData, string publicKey, Encoding encode)
        {
            byte[] DataToEncrypt = encode.GetBytes(resData);

            RSA rsa = DecodePemPublicKey(publicKey);
            byte[] result = rsa.Encrypt(DataToEncrypt, RSAEncryptionPadding.OaepSHA1);
        ...
        public static string decryptData(string resData, string privateKey)
        {
            ...
            for (int j = 0; j < DataToDecrypt.Length / 128; j++)
            {
                ...
                result.AddRange(decrypt(buf, privateKey, AlipayConfig.Input_charset));
            }
            byte[] source = result.ToArray();
            char[] asciiChars = new char[Encoding.GetEncoding(AlipayConfig.Input_charset).GetCharCount(source, 0, source.Length)];
            Encoding.GetEncoding(AlipayConfig.Input_charset).GetChars(source, 0, source.Length, asciiChars, 0);
        ...
        private static RSA DecodePemPrivateKey(String pemstr)
        {
            ...
            if (pkcs8privatekey != null)
            {
                ...
                RSA rsa = DecodePrivateKeyInfo(pkcs8privatekey);
                return rsa;
            }
            else
                return null;
        ...
        private static RSA DecodePrivateKeyInfo(byte[] pkcs8)
        {
            ...
            try
            {
                ...
                RSA rsacsp = DecodeRSAPrivateKey(rsaprivkey);
                return rsacsp;
            }
